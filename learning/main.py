...
import torchnet as tnt   # 用于 torch 的代码复用和模块化编程的框架

DIR_PATH = os.path.dirname(os.path.realpath(__file__))
sys.path.insert(0, os.path.join(DIR_PATH,'..'))

from learning import spg   # point cloud network
...
def main():
    ...
    if (args.dataset=='sema3d' and args.db_test_name.startswith('test')) or (args.dataset.startswith('s3dis_02') and args.cvfold==2):
        ...
        torch.backends.cudnn.enabled = False   # test阶段不需要cudnn加速
    ...
    ptnCloudEmbedder = pointnet.CloudEmbedder(args)   # LPE?
    ...
    def train():
        ...
        if logging.getLogger().getEffectiveLevel() > logging.DEBUG: loader = tqdm(loader, ncols=65)   # see Line 131

        loss_meter = tnt.meter.AverageValueMeter()   # 计算平均损失
        acc_meter = tnt.meter.ClassErrorMeter(accuracy=True)   # 预测值和真实值的错误率
        confusion_matrix = metrics.ConfusionMatrix(dbinfo['classes'])   # n*n矩阵
        t0 = time.time()
        ...
        for bidx, (targets, GIs, clouds_data) in enumerate(loader):
            ...
            loss.backward()   # 语义分割网络反向传播
            ptnCloudEmbedder.bw_hook()   # 超点网络反向传播
    ...
    def eval_final():
        ...
        acc_meter = tnt.meter.ClassErrorMeter(accuracy=True)   # 计算分类任务中的错误率
        confusion_matrix = metrics.ConfusionMatrix(dbinfo['classes'])
...
def create_model(args, dbinfo):
    ...
    nfeat = args.ptn_widths[1][-1]   # 特征数量
