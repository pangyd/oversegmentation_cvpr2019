...
class STNkD(nn.Module):
    """
    Spatial Transformer Net for PointNet, producing a KxK transformation matrix. -- STN
    Parameters:
      nfeat: number of input features
      nf_conv: list of layer widths of point embeddings (before maxpool)   Conv2d layer
      nf_fc: list of layer widths of joint embeddings (after maxpool)   Linear layer
    """
    def __init__(self, nfeat, nf_conv, nf_fc, K=2, norm = 'batch', affine = True, n_group = 1):
        ...
        for i in range(len(nf_conv)):
            ...
            if norm == 'batch':
                ...
                modules.append(nn.GroupNorm(1,nf_conv[i]))   # like BatchNorm(),分组normalized
            elif norm == 'group':
        ...
        self.proj = nn.Linear(nf_fc[-1], K*K)   # 输出K*K维特征
...
class PointNet(nn.Module):
    """
    PointNet with only one spatial transformer and additional "global" input concatenated after maxpool.   PTN
    Parameters:
    ...
    def __init__(self, nf_conv, nf_fc, nf_conv_stn, nf_fc_stn, nfeat, nfeat_stn=2, nfeat_global=1, prelast_do=0.5, last_ac=False, is_res=False, norm = 'batch', affine = True, n_group = 1, last_bn = False):
        ...
        if is_res: #init with small number so that at first the residual pointnet is close to zero   why???
            # 当设置大值时，会覆盖上一层的结果，从而达不到残差学习的效果
    ...
    def forward(self, input, input_global):
        if self.nfeat_stn > 0:
            T = self.stn(input[:,:self.nfeat_stn,:])   # T.shape=(n, k, k)=(n, 2, 2)
            # torch.bmm(a,b) --> a(3,4,2)*b(3,2,4)=(3,4,4)
            xy_transf = torch.bmm(input[:,:2,:].transpose(1,2), T).transpose(1,2)   # 绕z轴rotate
...
class LocalCloudEmbedder():
    ...
    def __init__(self, args):
        ...
        self.stn_as_global = args.stn_as_global   # 利用stn学习全局特征
        
    def run_batch(self, model, clouds, clouds_global, *excess):
        """
        求解超点
        :param model:模型的类对象，可调用STN和PTN
        :param clouds:局部邻居点云
        :param clouds_global:全局点云
        :param excess:
        :return:标准化后的超点
        """
        ...
        if self.nfeat_stn > 0:
            T = model.stn(clouds[:batch_size,:self.nfeat_stn,:])   # 对每个batch_size的前nfeat_stn个特征进行stn
            for i in range(1,n_batches+1):
                T = torch.cat((T,model.stn(clouds[i * batch_size:(i+1) * batch_size,:self.nfeat_stn,:])))
            xy_transf = torch.bmm(clouds[:,:2,:].transpose(1,2), T).transpose(1,2)   # (n, 2, k-neighbors=2)
            clouds = torch.cat([xy_transf, clouds[:,2:,:]], 1)
            if self.stn_as_global:
                clouds_global = torch.cat([clouds_global, T.view(-1,4)], 1)   # concat全局特征和stn后的特征, K=2,K*K=4
