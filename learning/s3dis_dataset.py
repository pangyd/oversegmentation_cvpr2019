...
def get_info(args):
    ...
    if args.loss_weights == 'none':
        weights = np.ones((13,),dtype='f4')   # f4 == float32
    else:
        weights = h5py.File(args.S3DIS_PATH + "/parsed/class_count.h5")["class_count"][:].astype('f4')   # h5文件数据
    ...
    for n in range(1,7):
        ...
        for file in os.listdir(pathC):
            ...
            if file.endswith(".h5"):
                ...
                label_count = np.bincount(hard_labels, minlength=13)   # [0, max(hard_labels)]中，每个元素出现的次数
                ...
                P = np.concatenate([xyz, rgb, e[:,np.newaxis], lpsv, xyzn, distance_to_center[:,None]], axis=1)   # 总特征
