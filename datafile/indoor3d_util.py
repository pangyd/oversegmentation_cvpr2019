...
def collect_point_label(anno_path, out_filename, file_format='txt'):
    ...
    for f in glob.glob(os.path.join(anno_path, '*.txt')):
        cls = os.path.basename(f).split('_')[0]   # label name
...
def point_label_to_obj(input_filename, out_filename, label_color=True, easy_view=False, no_wall=False):
    """ For visualization of a room from data_label file,
	    ...
	    out_filename: OBJ filename, visualize input file by coloring point with label color
...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
    ...
    limit = np.amax(data, 0)[0:3]   # x, y, z的最大值
    ...
    return np.concatenate(block_data_list, 0), np.concatenate(block_label_list, 0)   # 列拼接
